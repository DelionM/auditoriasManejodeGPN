...
$observacionesCincoSeis = $_POST["observacionesCincoSeis"] ?? null;
$accionesCincoSeis = $_POST["accionesCincoSeis"] ?? null;
$problema = $_POST["idProblemasCincoSeis"] ?? null; // Corregido para coincidir con el JS
$estatusCincoSeis = $_POST["estatusCincoSeis"] ?? null;
$fecha_filaCincoSeis = $_POST["fecha_filaCincoSeis"] ?? null; // Corregido para coincidir con el JS
...
$query = "UPDATE auditorias SET 
    observacionesCincoSeis = ?, 
    accionesCincoSeis = ?, 
    idProblemasCincoSeis = ?, 
    estatusCincoSeis = ?, 
    fecha_filaCincoSeis = ?, 
    nombre_archivoCincoSeis = ?, 
    ruta_archivoCincoSeis = ?, 
    fecha_subidaCincoSeis = NOW()
    WHERE id_auditoria = ?";
...
if ($stmt->execute()) {
    if ($stmt->affected_rows > 0) {
        echo json_encode(["success" => true, "mensaje" => "✅ Fila 5.6 actualizada correctamente.", "id" => $id_auditoria]);
    } else {
        ...
        if ($insertStmt->execute()) {
            echo json_encode(["success" => true, "mensaje" => "✅ Fila 5.6 guardada correctamente.", "id" => $id_auditoria]);
        } else {

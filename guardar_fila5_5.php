...
$observacionesCincoCinco = $_POST["observacionesCincoCinco"] ?? null;
$accionesCincoCinco = $_POST["accionesCincoCinco"] ?? null;
$problema = $_POST["idProblemasCincoCinco"] ?? null; // Corregido para coincidir con el JS
$estatusCincoCinco = $_POST["estatusCincoCinco"] ?? null;
$fecha_filaCincoCinco = $_POST["fecha_filaCincoCinco"] ?? null; // Corregido para coincidir con el JS
...
$query = "UPDATE auditorias SET 
    ...
    nombre_archivoCincoCinco = ?, 
    ruta_archivoCincoCinco = ?, 
    fecha_subidaCincoCinco = NOW()
    WHERE id_auditoria = ?";
...
if ($stmt->execute()) {
    if ($stmt->affected_rows > 0) {
        echo json_encode(["success" => true, "mensaje" => "✅ Fila 5.5 actualizada correctamente.", "id" => $id_auditoria]);
    } else {
        ...
        if ($insertStmt->execute()) {
            echo json_encode(["success" => true, "mensaje" => "✅ Fila 5.5 guardada correctamente.", "id" => $id_auditoria]);
        } else {

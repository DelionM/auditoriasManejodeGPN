...
$observacionesCincoSiete = $_POST["observacionesCincoSiete"] ?? null;
$accionesCincoSiete = $_POST["accionesCincoSiete"] ?? null;
$problema = $_POST["idProblemasCincoSiete"] ?? null; // Corregido para coincidir con el JS
$estatusCincoSiete = $_POST["estatusCincoSiete"] ?? null;
$fecha_filaCincoSiete = $_POST["fecha_filaCincoSiete"] ?? null; // Corregido para coincidir con el JS
...
$query = "UPDATE auditorias SET 
    observacionesCincoSiete = ?, 
    accionesCincoSiete = ?, 
    idProblemasCincoSiete = ?, 
    estatusCincoSiete = ?, 
    fecha_filaCincoSiete = ?, 
    nombre_archivoCincoSiete = ?, 
    ruta_archivoCincoSiete = ?, 
    fecha_subidaCincoSiete = NOW()
    WHERE id_auditoria = ?";
...
if ($stmt->execute()) {
    if ($stmt->affected_rows > 0) {
        echo json_encode(["success" => true, "mensaje" => "✅ Fila 5.7 actualizada correctamente.", "id" => $id_auditoria]);
    } else {
        ...
        if ($insertStmt->execute()) {
            echo json_encode(["success" => true, "mensaje" => "✅ Fila 5.7 guardada correctamente.", "id" => $id_auditoria]);
        } else {

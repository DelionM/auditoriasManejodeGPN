...
$observacionesCincoUno = $_POST["observacionesCincoUno"] ?? null;
$accionesCincoUno = $_POST["accionesCincoUno"] ?? null;
$problema = $_POST["idProblemasCincoUno"] ?? null; // Corregido para coincidir con el JS
$estatusCincoUno = $_POST["estatusCincoUno"] ?? null;
$fechaFilaCincoUno = $_POST["fecha_filaCincoUno"] ?? null; // Corregido para coincidir con el JS
...
$query = "UPDATE auditorias SET 
    ...
    ruta_archivoCincoUno = ?, 
    fecha_subidaCincoUno = NOW()
    WHERE id_auditoria = ?";
...
if ($stmt->execute()) {
    if ($stmt->affected_rows > 0) {
        echo json_encode(["success" => true, "mensaje" => "✅ Fila 5.1 actualizada correctamente.", "id" => $id_auditoria]);
    } else {
        ...
        if ($insertStmt->execute()) {
            echo json_encode(["success" => true, "mensaje" => "✅ Fila 5.1 guardada correctamente.", "id" => $id_auditoria]);
        } else {
